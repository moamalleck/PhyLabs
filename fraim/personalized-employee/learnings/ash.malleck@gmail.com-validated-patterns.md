...
Using `browser_run_code` with `page.evaluate()` and `shreddit-post` element queries successfully extracted verbatim pain quotes, usernames, and post URLs from Reddit when `browser_snapshot` returned sparse YAML. The fix was triggered by cross-checking that the page had actually loaded (title visible) before accepting the empty snapshot. This pattern is essential for any Reddit-based research and should be applied immediately without first attempting `browser_snapshot`.
...
## P-MED Recognising platform-level failure vs. query failure by checking domain relevance
...
In two separate moments during the prospect discovery session, the signal that stopped further retry attempts was that zero results shared the professional domain of the target segment — indicating the platform was returning noise, not that the query needed refinement. Trying additional keyword variants when no result is from the right context is wasted time. When results lack all domain relevance, treat it as a platform-level failure and pivot the channel rather than iterating the query.
